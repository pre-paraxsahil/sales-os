    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 self-start sm:self-auto shadow-xs"
        >
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
        {filterButtons.map((btn) => (
          <button
            ...
            className={cn(
              'rounded-full px-3 py-1 text-xs font-bold transition-all whitespace-nowrap border',
      ...
      {loading ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-xs">
          ...
          <p className="text-xs text-slate-500 font-medium">Loading chronological timeline...</p>
        </div>
      ) : error ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-xs">
          ...
          <p className="text-xs text-slate-500 font-medium">No interaction events found for this filter.</p>
        </div>
      ) : (
          ...
          {events.map((ev) => {
            ...
            return (
              <div key={ev.id} className="relative group">
                ...
                <div
                  className={cn(
                    'absolute -left-6 top-1 h-3.5 w-3.5 rounded-full ring-4 ring-white transition-transform group-hover:scale-110 shadow-xs',
                    cfg.ringColor
                  )}
                />

                {/* Event Card */}
                <div className="rounded-2xl border border-slate-200 bg-white p-4 hover:border-slate-300 transition-all space-y-2 shadow-xs">
                  {/* Card Header */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={cn('rounded-lg border px-2 py-0.5 text-[10px] font-bold flex items-center gap-1', cfg.color)}>
                        <Icon className="h-3 w-3" />
                        {cfg.label.toUpperCase()}
                      </span>

                      {ev.statusBadge && (
                        <span className="rounded-lg bg-slate-50 border border-slate-200 px-2 py-0.5 text-[10px] font-mono text-slate-700 font-bold">
                    ...
                    <div className="flex items-center gap-2 text-right whitespace-nowrap">
                      ...
                      {ev.eventType === 'CALL' && ev.metadata?.callId && onOpenCallDetails && (
                        <button
                          ...
                          className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-0.5 ml-1 font-bold"
                  ...
                  {hasMultipleLines && (
                    <button
                      ...
                      className="text-[11px] text-indigo-600 hover:text-indigo-800 font-bold flex items-center gap-0.5 pt-1"
                    >
                      {isExpanded ? (
                        <>
