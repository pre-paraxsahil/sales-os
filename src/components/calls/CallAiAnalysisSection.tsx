...
export const CallAiAnalysisSection: React.FC<CallAiAnalysisSectionProps> = ({
  ...
  return (
    <div className="mt-3 rounded-xl border border-indigo-100 bg-indigo-50/30 p-4 space-y-4">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-indigo-100 pb-3">
        <div className="flex items-center gap-2">
          <div className="rounded-lg p-1.5 bg-indigo-50 text-indigo-600 border border-indigo-100">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          {analysis ? (
            <button
              ...
              disabled={analyzing}
              className="flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-all shadow-xs disabled:opacity-50"
              title="Request a fresh analysis from OpenAI"
            >
              <RefreshCw className={cn('h-3.5 w-3.5 text-indigo-600', analyzing && 'animate-spin')} />
            ...
            <button
              ...
              disabled={analyzing}
              className="flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-indigo-500 transition-all shadow-sm disabled:opacity-50"
            >
          ...
          <div className="space-y-1">
            ...
            {isConfigError ? (
              <p className="text-[11px] text-amber-700">
                ...
                configure your <code className="bg-white px-1 py-0.5 rounded font-mono border border-amber-200">OPENAI_API_KEY</code> in the server environment.
              </p>
            ) : (
      ...
      {analysis && (
        <div className="space-y-4 text-xs">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3.5 space-y-2 shadow-xs">
            ...
            <p className="text-slate-900 leading-relaxed font-semibold">{analysis.summary}</p>
            {analysis.businessUnderstanding && (
              <p className="text-[11px] text-slate-600 border-t border-slate-100 pt-2 mt-2">
                <strong className="text-slate-800">Business Understanding: </strong>
                {analysis.businessUnderstanding}
              </p>
            )}
          </div>
                ...
                {conflicts.map((conflict, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <span className="text-amber-700 text-[10px] font-bold">Conflict Detected</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                      <div className="rounded bg-emerald-50/50 p-2 border border-emerald-100">
                        ...
                        <p className="text-slate-800">{conflict.existingValue}</p>
                      </div>
                      <div className="rounded bg-indigo-50/50 p-2 border border-indigo-100">
                        <span className="text-[10px] text-indigo-800 font-bold block mb-0.5">
                          New Call Extraction:
                        </span>
                        <p className="text-slate-800">{conflict.newValue}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2 pt-1">
                      <button
                        ...
                        className="rounded px-2.5 py-1 text-[11px] font-semibold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 transition-colors"
                      >
                      ...
                      <button
                        ...
                        disabled={applyingMemory}
                        className="rounded px-2.5 py-1 text-[11px] font-bold text-white bg-indigo-600 hover:bg-indigo-500 transition-colors shadow-xs"
                      >
                        Update to New Fact
                      </button>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3.5 space-y-2 shadow-xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                <Layers className="h-3.5 w-3.5 text-indigo-600" /> Customer Requirements
              </span>
              {analysis.requirements.length > 0 ? (
                <ul className="space-y-1.5">
                  {analysis.requirements.map((req, idx) => (
                    <li
                      key={idx}
                      className="rounded bg-slate-50 p-2 border border-slate-100 flex items-start justify-between gap-2"
                    >
                      <span className="text-slate-800 font-medium">{req.value}</span>
                      {renderTrustBadge(req.status, req.confidence)}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-[11px] text-slate-400 italic">No specific requirements stated.</p>
              )}
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3.5 space-y-2 shadow-xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                <AlertTriangle className="h-3.5 w-3.5 text-rose-600" /> Pain Points Identified
              </span>
                ...
                <ul className="space-y-1.5">
                  {analysis.painPoints.map((pp, idx) => (
                    <li
                      ...
                      className="rounded bg-slate-50 p-2 border border-slate-100 flex items-start justify-between gap-2"
                    >
                      <span className="text-slate-800 font-medium">{pp.value}</span>
                      {renderTrustBadge(pp.status, pp.confidence)}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-[11px] text-slate-400 italic">No pain points detected.</p>
              )}
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3.5 space-y-2 shadow-xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-800 flex items-center gap-1.5">
                <TrendingUp className="h-3.5 w-3.5 text-emerald-600" /> Buying Signals
              </span>
              {analysis.buyingSignals.length > 0 ? (
                <ul className="space-y-1.5">
                  {analysis.buyingSignals.map((bs, idx) => (
                    <li
                      key={idx}
                      className="rounded bg-emerald-50/60 p-2 border border-emerald-100 text-emerald-900 font-medium"
                    >
                      {bs.value}
                    </li>
                  ))}
                ...
                <p className="text-[11px] text-slate-400 italic">No explicit buying signals recorded.</p>
              )}
            </div>

            <div className="rounded-xl border border-slate-200 bg-white p-3.5 space-y-2 shadow-xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-amber-800 flex items-center gap-1.5">
                <ShieldAlert className="h-3.5 w-3.5 text-amber-600" /> Objections & Hesitations
              </span>
              {analysis.objections.length > 0 ? (
                <ul className="space-y-1.5">
                  {analysis.objections.map((obj, idx) => (
                    <li
                      key={idx}
                      className="rounded bg-amber-50/60 p-2 border border-amber-100 text-amber-900 font-medium"
                    >
                      {obj.value}
                    </li>
                  ))}
                ...
                <p className="text-[11px] text-slate-400 italic">No objections raised during this call.</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3 space-y-1 shadow-xs">
              ...
              <p className="text-xs font-bold text-slate-900">{analysis.budget.value}</p>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3 space-y-1 shadow-xs">
              ...
              <p className="text-xs font-bold text-slate-900">{analysis.timeline.value}</p>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3 space-y-1 shadow-xs">
              ...
              <p className="text-xs font-bold text-slate-900">{analysis.decisionMaker.value}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-3 space-y-1.5">
              ...
              <p className="text-slate-900 font-bold">{analysis.nextAction.value}</p>
              <div className="flex items-center gap-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3 space-y-1.5 shadow-xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1">
                <Info className="h-3 w-3 text-slate-400" /> Unknown / Missing Qualification
              </span>
              {analysis.unknownInformation.length > 0 ? (
                ...
                <p className="text-[11px] text-slate-400 italic">No missing information noted.</p>
              )}
            </div>
          </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3.5 space-y-3 shadow-xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-500 transition-all shadow-xs disabled:opacity-50 self-start sm:self-auto"
                >
                ...
                {candidates.map((cand, idx) => (
                  ...
                  >
                    <input
                      ...
                      className="mt-0.5 rounded border-slate-300 bg-white text-indigo-600 focus:ring-0"
                    />
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        {cand.hasConflict && (
                          <span className="text-[9px] text-amber-700 font-bold border border-amber-200 px-1 rounded bg-amber-50">
                            Conflict
                          </span>
                        )}
                      </div>
